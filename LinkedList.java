...
public class LinkedList<E extends Comparable<E>> {
	...
	private void remove(Node ref, E el) {
		...
		if (ref == first && el.equals(ref.data)) {
			first = first.next;
			remove(ref, el);
		}
		if (ref.next != null) {
	...
	private void duplicate(Node ref, E el) {
		if (this.first == null) {
			return;
		}
		if (ref == null) {
		...
		if (ref.next.next != null) {
			if (el.equals(ref.data)) {
				Node duplicated = new Node(el);
				duplicated.data = ref.data;
				if (ref.next != null) {
					duplicated.next = ref.next;
					ref.next = duplicated;
					n++;
				} else {
					return;
